...
SHADOW_CERTIFICATION_ENABLED = os.environ.get(
    "SHADOW_CERTIFICATION_ENABLED",
    "true",
).lower() in ("true", "1", "yes")
SHADOW_CERTIFICATION_LOOKBACK_DAYS = int(
    os.environ.get("SHADOW_CERTIFICATION_LOOKBACK_DAYS", 7)
)
SHADOW_CERTIFICATION_LIMIT_CYCLES = int(
    os.environ.get("SHADOW_CERTIFICATION_LIMIT_CYCLES", 0)
)
SHADOW_CERTIFICATION_REPORT_PATH = os.environ.get(
    "SHADOW_CERTIFICATION_REPORT_PATH",
    "reports/shadow_certification_report.json",
)
SHADOW_CERTIFICATION_MIN_SHADOW_TRADES = int(
    os.environ.get("SHADOW_CERTIFICATION_MIN_SHADOW_TRADES", 10)
)
SHADOW_CERTIFICATION_MAX_OPEN_GAP_RATIO = float(
    os.environ.get("SHADOW_CERTIFICATION_MAX_OPEN_GAP_RATIO", 0.40)
)
SHADOW_CERTIFICATION_MAX_REALIZED_PNL_GAP_RATIO = float(
    os.environ.get("SHADOW_CERTIFICATION_MAX_REALIZED_PNL_GAP_RATIO", 0.45)
)
SHADOW_CERTIFICATION_MAX_SLIPPAGE_DRIFT_BPS = float(
    os.environ.get("SHADOW_CERTIFICATION_MAX_SLIPPAGE_DRIFT_BPS", 2.0)
)
SHADOW_CERTIFICATION_MAX_DEGRADED_SOURCE_RATIO = float(
    os.environ.get("SHADOW_CERTIFICATION_MAX_DEGRADED_SOURCE_RATIO", 0.35)
)
SHADOW_CERTIFICATION_MAX_READINESS_INTERRUPTION_RUNS = int(
    os.environ.get("SHADOW_CERTIFICATION_MAX_READINESS_INTERRUPTION_RUNS", 0)
)
CAPITAL_RAMP_ENABLED = os.environ.get(
    "CAPITAL_RAMP_ENABLED",
    "true",
).lower() in ("true", "1", "yes")
CAPITAL_RAMP_INTERVAL_HOURS = float(
    os.environ.get("CAPITAL_RAMP_INTERVAL_HOURS", 24.0)
)
CAPITAL_RAMP_LOOKBACK_HOURS = float(
    os.environ.get("CAPITAL_RAMP_LOOKBACK_HOURS", 24.0 * 14)
)
CAPITAL_RAMP_DEFAULT_STAGE = os.environ.get(
    "CAPITAL_RAMP_DEFAULT_STAGE",
    "bootstrap",
).strip().lower()
CAPITAL_RAMP_APPROVED_STAGE = os.environ.get(
    "CAPITAL_RAMP_APPROVED_STAGE",
    CAPITAL_RAMP_DEFAULT_STAGE,
).strip().lower()
CAPITAL_RAMP_REQUIRE_SHADOW_CERTIFIED = os.environ.get(
    "CAPITAL_RAMP_REQUIRE_SHADOW_CERTIFIED",
    "true",
).lower() in ("true", "1", "yes")
CAPITAL_RAMP_REQUIRE_BENCHMARK_CLEAR = os.environ.get(
    "CAPITAL_RAMP_REQUIRE_BENCHMARK_CLEAR",
    "true",
).lower() in ("true", "1", "yes")
CAPITAL_RAMP_MAX_PAPER_DRAWDOWN_PCT = float(
    os.environ.get("CAPITAL_RAMP_MAX_PAPER_DRAWDOWN_PCT", 0.12)
)
CAPITAL_RAMP_MAX_LIVE_DRAWDOWN_PCT = float(
    os.environ.get("CAPITAL_RAMP_MAX_LIVE_DRAWDOWN_PCT", 0.08)
)
CAPITAL_RAMP_MIN_LIVE_SNAPSHOTS = int(
    os.environ.get("CAPITAL_RAMP_MIN_LIVE_SNAPSHOTS", 12)
)
CAPITAL_RAMP_MAX_SLIPPAGE_DRIFT_BPS = float(
    os.environ.get("CAPITAL_RAMP_MAX_SLIPPAGE_DRIFT_BPS", 2.0)
)
CAPITAL_RAMP_MAX_DEGRADED_SOURCE_RATIO = float(
    os.environ.get("CAPITAL_RAMP_MAX_DEGRADED_SOURCE_RATIO", 0.30)
)
CAPITAL_RAMP_MAX_REALIZED_PNL_GAP_RATIO = float(
    os.environ.get("CAPITAL_RAMP_MAX_REALIZED_PNL_GAP_RATIO", 0.45)
)
CAPITAL_RAMP_BOOTSTRAP_ORDER_CAP_MULTIPLIER = float(
    os.environ.get("CAPITAL_RAMP_BOOTSTRAP_ORDER_CAP_MULTIPLIER", 0.35)
)
CAPITAL_RAMP_TRIAL_ORDER_CAP_MULTIPLIER = float(
    os.environ.get("CAPITAL_RAMP_TRIAL_ORDER_CAP_MULTIPLIER", 0.55)
)
CAPITAL_RAMP_SCALED_ORDER_CAP_MULTIPLIER = float(
    os.environ.get("CAPITAL_RAMP_SCALED_ORDER_CAP_MULTIPLIER", 0.80)
)
CAPITAL_RAMP_FULL_ORDER_CAP_MULTIPLIER = float(
    os.environ.get("CAPITAL_RAMP_FULL_ORDER_CAP_MULTIPLIER", 1.00)
)
CAPITAL_RAMP_BOOTSTRAP_SOURCE_CAP_MULTIPLIER = float(
    os.environ.get("CAPITAL_RAMP_BOOTSTRAP_SOURCE_CAP_MULTIPLIER", 0.40)
)
CAPITAL_RAMP_TRIAL_SOURCE_CAP_MULTIPLIER = float(
    os.environ.get("CAPITAL_RAMP_TRIAL_SOURCE_CAP_MULTIPLIER", 0.60)
)
CAPITAL_RAMP_SCALED_SOURCE_CAP_MULTIPLIER = float(
    os.environ.get("CAPITAL_RAMP_SCALED_SOURCE_CAP_MULTIPLIER", 0.80)
)
CAPITAL_RAMP_FULL_SOURCE_CAP_MULTIPLIER = float(
    os.environ.get("CAPITAL_RAMP_FULL_SOURCE_CAP_MULTIPLIER", 1.00)
)
CAPITAL_RAMP_REPORT_PATH = os.environ.get(
...
ADAPTIVE_PROMOTION_SCALED_MIN_CLOSED_TRADES = int(
    os.environ.get("ADAPTIVE_PROMOTION_SCALED_MIN_CLOSED_TRADES", 8)
)
ADAPTIVE_PROMOTION_SCALED_MIN_RECENT_CLOSED_TRADES = int(
    os.environ.get("ADAPTIVE_PROMOTION_SCALED_MIN_RECENT_CLOSED_TRADES", 2)
)
ADAPTIVE_PROMOTION_SCALED_MIN_HEALTH_SCORE = float(
    os.environ.get("ADAPTIVE_PROMOTION_SCALED_MIN_HEALTH_SCORE", 0.58)
)
ADAPTIVE_PROMOTION_SCALED_MIN_RECENT_WIN_RATE = float(
    os.environ.get("ADAPTIVE_PROMOTION_SCALED_MIN_RECENT_WIN_RATE", 0.52)
)
ADAPTIVE_PROMOTION_SCALED_MIN_RECENT_RETURN_PCT = float(
    os.environ.get("ADAPTIVE_PROMOTION_SCALED_MIN_RECENT_RETURN_PCT", 0.0)
)
ADAPTIVE_PROMOTION_FULL_MIN_CLOSED_TRADES = int(
    os.environ.get("ADAPTIVE_PROMOTION_FULL_MIN_CLOSED_TRADES", 16)
)
ADAPTIVE_PROMOTION_FULL_MIN_RECENT_CLOSED_TRADES = int(
    os.environ.get("ADAPTIVE_PROMOTION_FULL_MIN_RECENT_CLOSED_TRADES", 5)
)
ADAPTIVE_PROMOTION_FULL_MIN_HEALTH_SCORE = float(
    os.environ.get("ADAPTIVE_PROMOTION_FULL_MIN_HEALTH_SCORE", 0.72)
)
ADAPTIVE_PROMOTION_FULL_MIN_RECENT_WIN_RATE = float(
    os.environ.get("ADAPTIVE_PROMOTION_FULL_MIN_RECENT_WIN_RATE", 0.56)
)
ADAPTIVE_PROMOTION_FULL_MIN_RECENT_RETURN_PCT = float(
    os.environ.get("ADAPTIVE_PROMOTION_FULL_MIN_RECENT_RETURN_PCT", 0.003)
)
ADAPTIVE_PROMOTION_FULL_MIN_LIVE_SUCCESS_RATE = float(
    os.environ.get("ADAPTIVE_PROMOTION_FULL_MIN_LIVE_SUCCESS_RATE", 0.55)
)
ADAPTIVE_PROMOTION_INCUBATING_MULTIPLIER = float(
    os.environ.get("ADAPTIVE_PROMOTION_INCUBATING_MULTIPLIER", 0.55)
)
ADAPTIVE_PROMOTION_TRIAL_MULTIPLIER = float(
    os.environ.get("ADAPTIVE_PROMOTION_TRIAL_MULTIPLIER", 0.75)
)
ADAPTIVE_PROMOTION_SCALED_MULTIPLIER = float(
    os.environ.get("ADAPTIVE_PROMOTION_SCALED_MULTIPLIER", 0.95)
)
ADAPTIVE_PROMOTION_FULL_MULTIPLIER = float(
    os.environ.get("ADAPTIVE_PROMOTION_FULL_MULTIPLIER", 1.10)
)
ADAPTIVE_PROMOTION_INCUBATING_CAP_PCT = float(
    os.environ.get("ADAPTIVE_PROMOTION_INCUBATING_CAP_PCT", 0.08)
)
ADAPTIVE_PROMOTION_TRIAL_CAP_PCT = float(
    os.environ.get("ADAPTIVE_PROMOTION_TRIAL_CAP_PCT", 0.14)
)
ADAPTIVE_PROMOTION_SCALED_CAP_PCT = float(
    os.environ.get("ADAPTIVE_PROMOTION_SCALED_CAP_PCT", 0.22)
)
ADAPTIVE_PROMOTION_FULL_CAP_PCT = float(
    os.environ.get("ADAPTIVE_PROMOTION_FULL_CAP_PCT", 0.32)
)
ADAPTIVE_RECALIBRATION_ENABLED = os.environ.get(
    "ADAPTIVE_RECALIBRATION_ENABLED",
    "true",
).lower() in ("true", "1", "yes")
ADAPTIVE_RECALIBRATION_INTERVAL_HOURS = float(
    os.environ.get("ADAPTIVE_RECALIBRATION_INTERVAL_HOURS", 24.0)
)
ADAPTIVE_RECALIBRATION_PROMOTION_CONFIRM_RUNS = int(
    os.environ.get("ADAPTIVE_RECALIBRATION_PROMOTION_CONFIRM_RUNS", 2)
)
ADAPTIVE_RECALIBRATION_DEMOTION_CONFIRM_RUNS = int(
    os.environ.get("ADAPTIVE_RECALIBRATION_DEMOTION_CONFIRM_RUNS", 2)
)
ADAPTIVE_RECALIBRATION_COOLDOWN_HOURS = float(
    os.environ.get("ADAPTIVE_RECALIBRATION_COOLDOWN_HOURS", 24.0)
)
ADAPTIVE_RECALIBRATION_IMMEDIATE_BLOCK_DEMOTION = os.environ.get(
    "ADAPTIVE_RECALIBRATION_IMMEDIATE_BLOCK_DEMOTION",
    "true",
).lower() in ("true", "1", "yes")
...
SOURCE_BUDGET_CAPITAL_RAMP_ENABLED = os.environ.get(
    "SOURCE_BUDGET_CAPITAL_RAMP_ENABLED",
    "true",
).lower() in ("true", "1", "yes")
SOURCE_BUDGET_PROMOTION_LADDER_ENABLED = os.environ.get(
    "SOURCE_BUDGET_PROMOTION_LADDER_ENABLED",
    "true",
).lower() in ("true", "1", "yes")
...
ARENA_CAPITAL_CHAMPION_WEIGHT = float(
    os.environ.get("ARENA_CAPITAL_CHAMPION_WEIGHT", 1.45)
)
ARENA_CAPITAL_ACTIVE_WEIGHT = float(
    os.environ.get("ARENA_CAPITAL_ACTIVE_WEIGHT", 1.00)
)
ARENA_CAPITAL_INCUBATING_WEIGHT = float(
    os.environ.get("ARENA_CAPITAL_INCUBATING_WEIGHT", 0.70)
)
ARENA_CAPITAL_PROBATION_WEIGHT = float(
    os.environ.get("ARENA_CAPITAL_PROBATION_WEIGHT", 0.45)
)
ARENA_CAPITAL_MIN_TRACK_RECORD_TRADES = int(
    os.environ.get("ARENA_CAPITAL_MIN_TRACK_RECORD_TRADES", 5)
)
...
DISCOVERY_STARVATION_RECOVERY_INTERVAL = int(
    os.environ.get("DISCOVERY_STARVATION_RECOVERY_INTERVAL", 3600)
)
DISCOVERY_STARTUP_RECOVERY_DELAY = int(
    os.environ.get("DISCOVERY_STARTUP_RECOVERY_DELAY", 300)
)
DISCOVERY_RECOVERY_MIN_ACTIVE_TRADERS = int(
    os.environ.get("DISCOVERY_RECOVERY_MIN_ACTIVE_TRADERS", 20)
)
DISCOVERY_RECOVERY_MIN_ACTIVE_STRATEGIES = int(
    os.environ.get("DISCOVERY_RECOVERY_MIN_ACTIVE_STRATEGIES", 25)
)
