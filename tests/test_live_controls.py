...
def test_sync_shadow_book_skips_unmirrored_paper_trade(monkeypatch):
    ...
    monkeypatch.setattr(
        "src.core.live_execution.db.get_open_paper_trades",
        lambda: [{"id": 99, "coin": "ETH", "side": "long", "entry_price": 2000.0}],
    )
    monkeypatch.setattr("src.core.live_execution.get_all_mids", lambda: {"ETH": 2100.0})
    monkeypatch.setattr(
        "src.core.live_execution.db.close_paper_trade",
        lambda trade_id, exit_price, pnl: closed.append((trade_id, exit_price, pnl)) or True,
    )
...
def test_daily_pnl_refresh_counts_fees_and_uses_fresh_positions(monkeypatch):
    seen_force_fresh = []
    ...
    class FillApiManager:
        def post(self, payload, *args, **kwargs):
            if payload.get("type") == "userFills":
                now_ms = int(datetime.now(timezone.utc).timestamp() * 1000)
                return [
                    {"time": now_ms, "closedPnl": "-1.00", "fee": "0.25"},
                    {"time": now_ms, "closedPnl": "0.50", "fee": "0.10"},
                ]
            return {}
    ...
    trader = LiveTrader(firewall=FakeFirewall(), dry_run=False, max_order_usd=1_000_000)
    trader.api_manager = FillApiManager()

    assert trader.update_daily_pnl_from_fills() is True
    assert seen_force_fresh == [True]
    assert trader.daily_realized_pnl == pytest.approx(-0.85)
    assert trader.daily_fees_paid == pytest.approx(0.35)
    assert trader.daily_unrealized_pnl == pytest.approx(-3.5)
    assert trader.daily_pnl == pytest.approx(-4.35)
