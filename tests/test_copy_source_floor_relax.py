...
def test_full_revert_to_legacy_floor_requires_disabling_both_levers():
    # Defense-in-depth: the deadlock is broken if EITHER lever is on.
    # main (A1-A7 lineage) independently flipped RELAX_ENABLED default ON
    # for the same structural reason as #1, so reverting to the legacy
    # floor needs BOTH exemption and relax disabled.
...
def test_exempt_disabled_alone_still_breaks_deadlock_via_relax_default():
    # With only exemption disabled, main's default-ON relax still clears
    # the structural dead-zone (0.45 - 0.07 = 0.38; conf 0.42 passes).
    fw = _fw(copy_source_floor_synthetic_exempt_enabled=False)
