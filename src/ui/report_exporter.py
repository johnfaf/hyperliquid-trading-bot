...
def export_html_report(output_dir: str = None) -> str:
    ...
    * {
    ...
    if equity_curve:
        ...
        html.append("      label: 'Account Balance',")
        html.append(f"      data: {json.dumps(balances)},")
        html.append("      borderColor: '#00d4ff',")
        html.append("      backgroundColor: 'rgba(0, 212, 255, 0.1)',")
        ...
        html.append("    plugins: {")
        html.append("      legend: { labels: { color: '#a0a0a0' } }")
        html.append("    },")
        html.append("    scales: {")
        html.append("      y: { ticks: { color: '#a0a0a0' } },")
        html.append("      x: { ticks: { color: '#a0a0a0' } }")
        html.append("    }")
        html.append("  }")
        html.append("});")
